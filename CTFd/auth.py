...
def register():
    ...
    if request.method == "POST":
        ...
        stu_id_long = len(password) > 128
        stu_name_long = len(password) > 128
        stu_class_long = len(password) > 128
        ...
        if pass_long:
            errors.append("密码过长")
        if stu_id_long or stu_name_long or stu_class_long:
            errors.append("姓名/学号/年级班级 有误")
        if name_len:
        ...
        if len(errors) > 0:
            ...
            with app.app_context():
                user = Users(name=name, email=email_address, password=password, stu_name=stu_name, stu_id=stu_id, stu_class=stu_class)
